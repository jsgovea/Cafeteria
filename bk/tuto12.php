<?php
			...
			include("conectarse.php");
  			$link=Conectarse();
  			$result5 = mysql_query('SELECT max(NumOrden) as NumOrden FROM orden_temp');	
  ...
  $q = "UPDATE orden_temp set NumOrden='".$neworden."' where NumOrden='".$espacio."' AND user='".$nombre."'";
	...
	$result2 = mysql_query('SELECT * FROM orden_temp WHERE user=\''.$nombre.'\' AND NumOrden=\''.			$neworden.'\''); 
...
{
    //Logo
    $this->Image('MYTEK.JPG',15,12,70);
    //Arial bold 15
    $this->SetFont('Arial','B',12);
    //Movernos a la derecha
    $this->Cell(80);
    //Título
    $this->Cell(110,10,'MERRYTECH INTERNACONAL, S.A. DE C.V. ',0,0,'C');
	$this->Ln(1);
...
function Footer()
{
	...
	//Creación del objeto de la clase heredada
		...
		$pdf->SetFont('Arial','B',9);
			...
			$pdf->SetFont('Arial','',9);
			$pdf->Cell(25);
			$pdf->Cell(10,10,$Facturar2."-00".$neworden);	
			$pdf->Ln(5);
			...
			IF($Facturar2=="MT")
			{
			$FACTURAR3="MERRYTECH INTERNACIONAL S.A. DE C.V.";
		...
		$pdf->SetFont('Arial','b',9);
			...
			$pdf->Cell(84);	
			$pdf->SetFont('Arial','',6);
	        $pdf->Cell(10,10,'CALLE COCHIMIES NO. 18480, INT. 821-D,   ');
				$pdf->Ln(3);
				$pdf->Cell(107);	
			 $pdf->Cell(10,10,'COL. GUAYCURA, TIJUANA B.C. , MEXICO');	
...
switch ($iva) {
	...
	else
	{
	 ...
	 $cadena5=$cadena4."0";
			  ...
			  $pdf->Ln(10);
			  $pdf->Cell(70);
			  $pdf->Cell(10,10,'MERRYTECH INTERNACTIONAL S.A. DE C.V.');
			  $pdf->Ln(10);			
			  $pdf->SetFont('Arial','',9);	
			  $pdf->Cell(10,10,'***NOTA:***');
		      $pdf->Ln(5);
